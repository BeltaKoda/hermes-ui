...
interface PaneChrome extends PaneSizing {
  ...
  collapsible?: boolean
  /** Web-only responsive extension for a pane that stays tiled on Desktop but
   *  belongs in an edge drawer on a phone (for example Bot Mode Cronjobs). */
  narrowCollapsible?: boolean
  /** Responsive drawer edge when it differs from the Desktop placement. */
...
/** Desktop's `collapsible` contract plus the web-only phone extension. */
export const paneUsesNarrowOverlay = (c: Contribution | undefined): boolean => {
  const chrome = paneChrome(c)

  return Boolean(chrome.collapsible || chrome.narrowCollapsible)
}

export const paneNarrowSide = (c: Contribution): 'left' | 'right' => {
  const chrome = paneChrome(c)

  return chrome.narrowSide ?? (chrome.placement === 'left' ? 'left' : 'right')
}
