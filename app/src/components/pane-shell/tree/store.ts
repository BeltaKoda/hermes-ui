...
import { paneUsesNarrowOverlay, rootChildSide } from './renderer/track-model'
...
export function applyTree(tree: LayoutNode, presetId: string) {
  ...
  // An opener may need to turn a pane's backing store on, but that should not
  // let it steal the active tab chosen by the preset. Focus deliberately puts
  // workspace first in a shared workspace/files/review/terminal group.
  const restorePresetActives = (presetNode: LayoutNode) => {
    if (presetNode.type === 'group') {
      const live = $layoutTree.get()

      if (live && presetNode.active) {
        const next = setActivePaneOp(live, presetNode.id, presetNode.active)
    ...
    presetNode.children.forEach(restorePresetActives)
  }

  restorePresetActives(tree)
}
